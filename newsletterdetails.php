...
?>
               ...
               <? if(isset($_POST['Reguser']))
                 {
                   ...
                   $list =array();
                   $json_object = array();
                   //get all the resitered user
                   $AllRegUsers=GetAllReguser($todate);
                   while ($reguser_row = mysql_fetch_array($AllRegUsers))
                   {
                    $UserhasTreesql= "select  *  from  user_tree_table as utt,trees as t  where user_id='$reguser_row[user_id]' and t.tree_Id=utt.tree_Id and deleted = 0   ";
                    $dbc=Dbconn::getdblink();
                    $Usertrees = $dbc->readtabledata($UserhasTreesql);
                    $NoOfTrees=mysql_num_rows($Usertrees);
                    if ($NoOfTrees > 0)
                    {
                        while ($regusertree_row = mysql_fetch_array($Usertrees))
                        {
                            //get the nunber of observation for this tree_id
                            $Usertreewithobsql="select * from user_tree_observations where user_id='$reguser_row[user_id]'
                           ...
                           // echo $Usertreewithobsql;
                            ...
                            if ($TreewithObno >0)
                            {
                                ...
                                if($reguser_row['group_id'] >0)
                                {
                                    $schoolinfo="select group_name from user_groups where group_id='$reguser_row[group_id]'
                                    and coord_id='$reguser_row[user_id]'";
                                    //echo $schoolinfo;
                                    $schoolname = $dbc->readtabledata($schoolinfo);
                                    list($group_name) = mysql_fetch_row($schoolname);
                                ...
                                else
                                {
                                 ...
                                 $info= $reguser_row['user_id'].",".$reguser_row['user_category'].",".
                                                 $reguser_row['user_email'].",".$reguser_row['group_id'].",".$group_name.",".
                                                 $reguser_row['full_name'].",".$reguser_row['state_id'].",".
                                                 $reguser_row['date_of_addition'].",".$species_primary_common_name.",".
                                                 $regusertree_row['tree_nickname'].",".$regusertree_row['user_tree_id'].",".
                                                 $regusertree_row['last_observation_date'].",".$TreewithObno;
                                          $list[]=$info;
                        ...
                        $file = fopen("Allreguserwithonetreeoneob.csv","w");
                        ...
                        fclose($file);
                        if (file_exists("Allreguserwithonetreeoneob.csv"))
                    { //check indivuserwithtreeob.csv
                        ?>
                             
                              All Reg User(with atleast One tree and one observation) file created <a href="Allreguserwithonetreeoneob.csv" style="color:red">download file "indivuserwithtreeob.csv" </a>
                              </br>
                ...
                {   $i=0;  
                    ...
                    while ($regusertree_row = mysql_fetch_array($Usertrees))
                    {
                       ...
                       $userInformation[$i]=$info;
                          $i++;
                          //echo "<br>";
                            //$jenc = json_encode($info);
                             //$row['tree_nickname'] ,$row['species_primary_common_name'],$row['city'],$row['location_name']);
                                                        //echo $jenc;
                ...
                if(isset($_POST['Seedtreeuser']))
                {
                    ...
                    $Userlist[0]= $user_id.",".$user_category.",".$user_email.","
                    ...
                    $indusersql="select distinct users.user_id,users.user_category,users.user_email,users.group_id,users.full_name,
                        users.state_id,users.date_of_addition,users.date,users.group_id from users where users.user_category='school-seed' and user_id!='140' and date_of_addition  <='$todate'";
                    //echo $indusersql;
                    $dbc=Dbconn::getdblink();
                    ...
                    while ($induser_row = mysql_fetch_array($induser))
                    {
                        ...
                        $indivtreeno=0;
                        $group_name="-";
                        //$IndUsertreesql="select * from trees,user_tree_table where trees.deleted = 0 and user_tree_table.user_id='$induser_row[user_id]' and trees.tree_Id= user_tree_table.tree_id and user_tree_table.user_id!=140";
                        ...
                        if ($NoOfTrees >'0')
                        {
                         ...
                         {
                              $treeentered=0;
                               $UsertreespeciesSql="select species_primary_common_name from  species_master where species_id='$indusertrees_row[species_id]'";
                            $usertreespecies = $dbc->readtabledata($UsertreespeciesSql);
                            list($species_primary_common_name)= mysql_fetch_row($usertreespecies);
                             //check whether observation exits or not
                              //no of observation for those trees 
                            ...
                            // if ($NoOfTreesob >'0')
                             //{
                                ...
                                if ($treeentered==0)
                                {
                                    $treeno++;
                                    $indivtreeno++;
                                    $full_name= str_replace(',', '-',  $induser_row['full_name']);
                                     $treenickname= str_replace(',', '-',  $indusertrees_row['tree_nickname']);
                                     $treeentered=1;
                                     $info= $induser_row['user_id'].",".$induser_row['user_category'].",".$induser_row['user_email'].","
                                    .$induser_row['group_id'].",".$group_name.",".$full_name.
                                    ",".$state.",".$induser_row['date_of_addition'].","
                                    .$species_primary_common_name.",".$treenickname.",".$indusertrees_row['user_tree_id'].","
                                    .$indusertrees_row['date_of_addition'].
                                    ",".$indusertrees_row['last_observation_date'].",".$NoOfTreesob;
                                     $Userlist[]=$info;
                               }
 ...
 if (file_exists($filename))
                ...
                { 
                     ...
                     getalluser($todate,"school",$filename);
                 }
                 if(isset($_POST['Swgspuser']))
                { 
                     $filename="gspusers.csv";
                     getalluser($todate,"school-gsp",$filename);
                 }
                  if(isset($_POST['SwIndiuser']))
                { 
                     $filename="indivusers.csv";
                     getalluser($todate,"individual",$filename);
                 }
                 
                 function getalluser($todate,$category,$filename)
                 {
                    $Userlist=array();
                    $data=array();
                    $Userlist=GetAllReguserwithcat($todate,$category);
                    $dbc=Dbconn::getdblink();
                    $userno =0;
                    $treeno=0;
                    $cntob=0;
                    ...
                    $date_of_addition="Registerd date";
                    $Nooftree = "No Trees";
                    $NoofObservation="No of Observations";
                    $data[0]= $user_id.",".$user_category.",".$user_email.","
                                        .$group_id.",".$group_name.",".$full_name.
                                          ",".$state_id.",".$date_of_addition.",".$Nooftree.",".$NoofObservation;
                    
                    
                    while ($induser_row = mysql_fetch_array($Userlist))
                    {
                        $userentered=0;
                        $treeentered=0;
                        $indivtreeno=0;
                        ...
                        if ($NoOfTrees >'0')
                        {
                            ...
                            while ($indusertrees_row = mysql_fetch_array($indusertrees))
                            {
                                ...
                                $Indusertreeob = $dbc->readtabledata($Indusertreeobsql);
                                $NoOfTreesob  = mysql_num_rows($Indusertreeob);
                                $indivTreesob = $NoOfTreesob;

                            }
                        } 
                        $data[]= $induser_row['user_id'].",".$induser_row['user_category'].",".$induser_row['user_email'].","
                                         .$induser_row['group_id'].",".$group_name.",".$full_name.
                                           ",".$state.",".$induser_row['date_of_addition'].",".$indivtreeno.",".$indivTreesob;
                ...
                { 
                   ...
                   <? }
                        ...
                        function Getuser($category)
                        {
                            ...
                            // echo $induser_row['user_id'];
                             // echo "   No Of Trees:-".$NoOfTrees;
                         while ($indusertrees_row = mysql_fetch_array($indusertrees))
                         {
                              $treeentered=0;
                               $UsertreespeciesSql="select species_primary_common_name from  species_master where species_id='$indusertrees_row[species_id]'";
