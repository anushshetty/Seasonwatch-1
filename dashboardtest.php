...
//check whether tree exists for the user 
...
{
	//alert("we are here");
	$treenum=Login::sanitize($_POST['treeno']);
	$observationdata    = array();
    $observationdata[0]  = Login::sanitize($_POST['obdate'.$treenum]);
    $observationdata[1]  = Login::sanitize($_POST['usertreeid'.$treenum]);
    ...
    $observationdata[19] = Login::sanitize($_SESSION['userid']);
       ...
       $addobstatus=$newobservation->AddObservation($dbc,$observationdata);
           ...
           if( $msg =="sucess"){
           	...
           	$_SESSION['msg']['addobserr']="success";
           	echo "<script type='text/javascript'>
                    $(document).ready(function(){
           document.getElementById('obssuccess').style.display='block';
           $('#obssuccess').delay(10000).fadeOut();
           //setTimeout( '$('obssuccess').hide();', 4000);
           ...
           /*	echo "<script type='text/javascript'>
           	...
           	//alert('Observation added successfully');
		  	    				/*function showmsg(){		  	    				
		  	    				$('#lightmsg').show();
		  	    				document.getElementById('preloader').innerHTML='".$msg."'
		  	    				}
		  	    				showmsg();* /
		  	    				});
		  	    				</script>";*/
           ...
           echo "<script type='text/javascript'>
		  	    				$(document).ready(function(){
		  	    				 document.getElementById('dateerrormsg'+".$treenum.").style.display='block';
    	document.getElementById('dateerrormsg'+".$treenum.").innerHTML='".$msg."'
    	document.getElementById('sucessinfo'+".$treenum.").style.display='none';
		                       document.getElementById('lightOne".$treenum."').style.display='block';
                          document.getElementById('fadeOne').style.display='block';window.scrollTo(0, 0);
...
/* this function is used to add treee */
function AddTreeInfo() {
	$("#button_area_ok a").style.backgroundColor='#C5C4C4';//#cc3300
	 document.getElementById('button_area_ok').disabled='true';
	 $("#button_area_ok a").style.backgroundColor='#C5C4C4';//#cc3300
...
if (selTree=='')
    {
        ...
        $('#lightSix').hide();
        $('#lightFour').show();
        document.getElementById('button_area_ok').disabled='false';
       // $("#button_area_ok a").style.backgroundColor='#cc3300';
        return false;
     }
var height;
...
//clear all the selction for add tree for seed category 
function clearseedaddtree()
{
	...
	$('ul.addtreeList li ul').hide();  
	document.getElementById('selspecies_id').value="";
        document.getElementById('selspecies_name').value="";	
	document.getElementById('boxDO').style.display='none';
	  ...
	  $("#q").val('');
          ...
          $("#zoominfo").val('');
        //clear all textbox in edit details
        document.getElementById('tree_nickname').value="";
        document.getElementById('tree_height').value="";
        document.getElementById('tree_girth').value="";
        document.getElementById('aspect').value="";
        document.getElementById('distance_from_water').value="";
        document.getElementById('degree_of_slope').value=""; 
 	document.getElementById('other_notes').value="";
        document.getElementById('studentname').value=""; 
        $('input:radio[name=is_watered]')[0].checked=false;
	$('input:radio[name=is_watered]')[1].checked=false;
        $('input:radio[name=is_fertilised]')[0].checked=false;
	$('input:radio[name=is_fertilised]')[1].checked=false;
        $('input:radio[name=tree_damage]')[0].checked=false;
	$('input:radio[name=tree_damage]')[1].checked=false;       

	initialize();
}
function cleargspaddtree()
{
...
document.getElementById('TBOX').className='tree_box1';
     ...
     //clear all selection 
      ...
      document.getElementById('speciescname').style.display='none';
      document.getElementById('specienotes').style.display='none';
    //  document.getElementById('pickmsg').style.display='none'; 
      //$("#pickmsg").text('');
      $("#selspeciesmsg").text('');
      $("#speciescname").val('');
...
{  window.scrollTo(0, 0);
	...
	// $('#dialog_edit_obs').css('top',  winH/8-$('#dialog_edit_obs').height()/2);
        ...
        else if (ubrowser=="Internet Explorer")
            {
         $('#dialog_edit_obs').css('top',  100);
         $('#dialog_edit_obs').css('left', winW/2-$('#dialog_edit_obs').width()/2);
            }
 ...
 function closeeditob()
    {
	
      $('#dialog_edit_obs').fadeOut(1000);$('#mask').fadeOut(500);
    }
...
{  
        ...
        $('a[name=openEditTreeModal]').click(function(e)
	{
        ...
        for (i=0;i<=treenums;i++)
        {
            ...
            $("#obdate"+i).datepicker({minDate: -noofDays, maxDate: '0M +0D', dateFormat: 'yy-mm-dd',autoclose: true});
        } 
...
<script type="text/javascript" >
function clearseededittree(dialogid)
    {
        //alert(id);
        document.getElementById("TBOX"+dialogid).style.display='block';
        document.getElementById("autotag"+dialogid).style.display='block';
        document.getElementById("pick"+dialogid).style.display='block';
        document.getElementById("edtreesel"+dialogid).className='cur';
        //diable edit location selection 
        document.getElementById("mapBox"+dialogid).style.display='none';
        document.getElementById("edlocation"+dialogid).className='cur';
        document.getElementById("eddetails"+dialogid).className='cur';
       //clear user selction and get back to old 
    }
    
    function cleargspedittree(dialogid)
    {
        //alert(id);
        document.getElementById("TBOX"+dialogid).style.display='block';
        document.getElementById("autotag"+dialogid).style.display='block';
 ...
 function TreenicknameChanged  (id)
 {
    document.getElementById("chnickname"+id).value=1;
 }   
function EditTree(id) {
var nick_name;
nick_name = trim(document.getElementById("etree_nickname"+id).value);
var nicknamech = document.getElementById("chnickname"+id).value;
var height;
...
function checkdateexists(ID,Isusertreeid){
	...
	//  alert(arr[i]);
		  if(chkobdate ==arr[i]){
	  	  ...
	  	  alert("Observation for this date already exists!");
		    document.getElementById("obdate"+ID).value="";
		  //document.getElementById("obdate"+ID).focus();
	    return false;
		}
	}//for
	document.getElementById("obdate"+ID).blur();
...
<div id="obssuccess" style="height: 42px;display:none"><img src="images/inform.png" style="height: 42px;width: 220px"/></div>
</div> <!-- #preloader -->
    <!--  start body_content  -->
    <div class="wrapper">
...
<div id="lightOne<?echo $i;?>" class="white_contentOne"><? include ("addobservation.php"); ?></div>
  ...
  <!--<li class="infoclose"><a href="" title="Close Tree Information."><img src="images/collapse.png"></a></li>-->
        ...
        <div id="lightSix<?echo $i;?>" class="white_contentThree"><?php if($_SESSION['usercategory'] == 'individual') {include ("indivedittree.php");}  elseif ($_SESSION['usercategory'] =='school-seed')
        {
             include ("seededittree.php");
        }else {include ("generaledittree.php");} ?>
                                                </div>
                                                              <?if ($totalnoofobservationscnt>0){?>
