...
{  
        ...
        $('a[name=openEditTreeModal]').click(function(e)
	{
        ...
        for (i=0;i<=treenums;i++)
        {
         ...
         $('#addobsclose').click(function(e)
	{
                e.preventDefault();
                var dlgid=this.name;
                document.getElementById("obdate"+dlgid).value="";
               	document.getElementById('addexits').style.display='none';
                document.getElementById('addsucess').style.display='none';
...
$.ajax({
type: "POST",
url: "trackobservationstest.php",
data: dataString,
success: function(data){
 var retdata=parseInt(data);
    if(retdata!=0)
    {

    $('.addexits').fadeIn(200).show();
    //window.setTimeout("$('dialogID').fadeOut(1000);$('#mask').fadeOut(500);$('.addexits').css('display','none');window.location.reload(true);", 2000);
    }
    else
    {
    $('.addsucess').fadeIn(200).show();
    $('.addexits').fadeOut(200).hide();
    window.setTimeout("$('dialogID').fadeOut(1000);$('#mask').fadeOut(500);$('.addsucess').css('display','none');window.location.reload(true);", 2000);
...
<body>
    ...
    <div class="body_content_2">
        ...
        //Malayam species name
        $maltreename = array(
        'Plavu' => 'Artocarpus heterophyllus',
        'Elengi'  => 'Mimusops elengi',
        'Katampu' => 'Neolamarckia cadamba',
        'Njaval'  => 'Syzygium cumini',
        'Atti'  => 'Ficus racemosa',
        'Aaval' => 'Holoptelea integrifolia',
        'Manimaruthu'  => 'Lagerstroemia speciosa',
        'Nelli' => 'Phyllanthus emblica',
        'Arayal' => 'Ficus religiosa',
        'Maavu'  => 'Ficus religiosa',
        'Kumbil'  => 'Gmelina arborea',
        'Vatta' => 'Macaranga peltata',
        'Paala' => 'Alstonia scholaris',
        'Ilippa'  => 'Madhuca longifolia',
        'Thekku' => 'Tectona grandis',
        'Mandaram' => 'Bauhinia purpurea',
        'Mullumurikku'  => 'Erythrina indica',
        'Koovalam' => 'Aegle marmelos',
        'Kanikonna,' => 'Cassia fistula',
        'Ungu'  => 'Pongamia pinnata',
        'Ashokam' => 'Saraca asoca',
        'Puli' => 'Tamarindus indica',
        'Gulmohur'  => 'Delonix regia',
        'Rain tree' => 'Samanea saman',
        'Mulilavu'=> 'Bombax ceiba'
       );
        ...
        { ?>
                    ...
                    <div class="body_top">
                        <div class="main">
                            ...
                            <!-- Get the list of trees from the database-->
                                    ...
                                    <div title="<?echo $group_trees_row['species_scientific_name']?>"><h5>
                                    <?$malayamname = array_search($group_trees_row['species_scientific_name'], $maltreename);
                            ...
                            <td style="height:51px;vertical-align:middle;padding:0px 15px;"><blockquote style="width:60px;">&nbsp;</blockquote></td>
                                ...
                                // to get the no of observations between start date & end date 
                               /* $qOb="SELECT  observation_id,date,freshleaf_count,matureleaf_count,bud_count,open_flower_count,fruit_ripe_count,fruit_unripe_count FROM user_tree_observations wHERE 
                                user_tree_id ='$group_trees_row[user_tree_id]' and user_id='$_SESSION[userid]' and deleted='0' and date>'".$start_date . "' AND date<='" . $end_date . "'ORDER BY date DESC";
                               // echo $qOb;
                                $observations=mysql_query($qOb) or die (mysql_error());;
                                $noofrowcol=mysql_num_rows($observations);
                                //echo $noofrowcol;
                                // number of observation after the start date to calculate the page no.
                                $q3="SELECT  observation_id,date,freshleaf_count,matureleaf_count,bud_count,open_flower_count,fruit_ripe_count,fruit_unripe_count FROM user_tree_observations wHERE 
                                user_tree_id ='$group_trees_row[user_tree_id]' and user_id='$_SESSION[userid]' and deleted='0' and date>'".$start_date . "'ORDER BY date DESC";
                                $obserafterdate=mysql_query($q3) or die (mysql_error());;
                                $nodateafter=mysql_num_rows($obserafterdate);

                                if($nodateafter<4){$obpageno="1";}
                                else {$obpageno=round(($nodateafter/4)+.4);}
                                
                                 if ($noofrowcol>0){
                                    ?>
                                <td style="width:71px;height:51px;vertical-align:middle;"><blockquote style="width:71px;text-align:center;color:#666666; font-size:13px;"><a href="#" title="Edit Observations of <?echo $monthdateformat;?>"
                                onclick="edit_obs_load('dash',<?echo $treeIDAr[$i]?>,<?echo $i?>,<?echo $obpageno?>,<?echo $obpageno?>,<?echo $noofrowcol;?>)"><img src="images/pencil.png" width="10px" height="10px"><?echo $monthdateformat;?></a></blockquote></td>
                                <?}
                                else{ ?>
                                ...
                                <?php include ("seedtreeinfotest.php"); ?>
            ...
            {?>
            <div class="Notreemsg">Welcome to SeaonWatch-Seed Programme Dashboard .You don't have any trees.ADD A TREE to watch.</div>
            <? }?>                   
 </div>
</div>
</div>
    ...
    <div id="fadeOne" class="black_overlayOneadd"></div>
     ...
     <!-- Add Observation Modal-->
        ...
        <?php 
        include ("addobservationtest.php"); 
        ?>
        </div>
        <?}?>
        <!--MODAL TREE INFO STARTS-->
        <div id="lightFour" class="white_contentThree">
        <?php //include ("seedtrees.php"); 
		 include ("seedtreesdream.php");
		?>
        </div>
        
        <!--add a tree start -->
        <!--<div id="lightSix" class="white_contentThree">-->
        <!--Add Tree modal starts-->
         <?php /*include ("seedaddtree.php");*/ ?>
        <!--Add Tree modal ends-->
        <!--</div>-->
        
       
         <? /* for ($j=0;$j<=count($treeIDAr);$j++) {?>
        <div id="lightSix<?echo $j;?>" class="white_contentThree">      
        <?php 
