...
$("#preloader").fadeOut(1000);
 ...
 function showtree(showtreeid)
 {
    // alert(showtreeid);
     ...
     url = 'inoftestmaps.php?id=' + showtreeid; 
...
if ($spid >0)
{
$spidtrees ="SELECT u.user_name, user_tree_table.last_observation_date,user_tree_table.tree_nickname,u.group_id, u.user_id, trees.species_id, sm.species_search_names, trees.tree_id, trees.tree_location_id, lm.city, lm.longitude, lm.latitude, lm.location_name
...
{  //$spid=0; 
$spidtrees = "SELECT u.user_name,u.group_id,user_tree_table.last_observation_date,u.user_id,user_tree_table.tree_nickname,trees.species_id,sm.species_search_names,trees.tree_id,trees.tree_location_id,lm.city,lm.longitude,lm.latitude
...
$resd1 = $dbc1->readtabledata($spidtrees);
//$resd1 =mysql_query($spidtrees);
$norows=mysql_num_rows($resd1);
...
{
	while( $row = mysql_fetch_array($resd1) )
	{ $icunt++;
             //echo     $row['tree_nickname']."-".$row['species_id'];
             //echo "<br>";
        $data[] = array($row['tree_nickname'] ,$row['species_search_names'],$row['city'],$row['location_name'],
  ...
  var shape = {
    ...
    for (i = 0; i < locations.length; i++) {  
      marker = new google.maps.Marker({
       ...
       var name = locations[i][0], species = locations[i][1], city = locations[i][2];
       var contentString = "";
        contentString = '<div style="margin-top:-5% !important;">'+
        '<h4>'+name+'</h4>'+
        'Species:-'+species+'</br>'+city+
         '</div>';
