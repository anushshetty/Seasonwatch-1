...
//$sql .= "order by tree_location_id DESC";
...
while ($row = @mysql_fetch_assoc($result))
{
  ...
  $final_description = "Species: " . $row['species_primary_common_name'] . "<br>Reported by: " .  $row['full_name'] . "<br>Location: " . $row['location_name'];

  $descNode = $dom->createElement('description', htmlentities($final_description));
  $placeNode->appendChild($descNode);
  $styleUrl = $dom->createElement('styleUrl', '#' . htmlentities($row['last_observation_date']) . 'Style');
...
echo $kmlOutput;
  ...
  $out .= "Species , Location name, City , State, Reporter, Date, Latitude,Longitude";
  $out .="\n";
  while ($row = mysql_fetch_array($result)) {
     $out .='"'.$row["species_primary_common_name"].'",';
  ...
  header("Content-Disposition: attachment; filename=Seasonwatch_reports.csv");
  echo $out;
